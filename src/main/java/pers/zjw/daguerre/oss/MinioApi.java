...
public class MinioApi extends AbstractOssApi {
    ...
    @Override
    public Collection<pers.zjw.daguerre.oss.domain.DeleteResult> delete(String bucket, Collection<String> objectNames)
            throws OssException {
        Assert.hasText(bucket, "bucket name must not be null");
        ...
        Iterable<Result<DeleteError>> results = client.removeObjects(
                RemoveObjectsArgs.builder()
                        ...
                        .build());
        Map<String, pers.zjw.daguerre.oss.domain.DeleteResult> map = new HashMap<>(objectNames.size());
        for (Result<DeleteError> result : results) {
            try {
                ...
                map.put(error.bucketName()+error.objectName(),
                        new pers.zjw.daguerre.oss.domain.DeleteResult(error.bucketName(), error.objectName(), true));
            } catch (ErrorResponseException | InsufficientDataException | InternalException
                    | InvalidKeyException | InvalidResponseException | IOException
                    | NoSuchAlgorithmException | ServerException | XmlParserException e) {
